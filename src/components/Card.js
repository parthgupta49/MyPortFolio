...
const Card = ({ card, isDescriptionShowing }) => {
  const key = Object.keys(card.image)
  return (
    <div className='flex flex-col items-center border border-[#7CA5B8] w-[350px] rounded-lg p-2'>
      <img src={card.image[key]} className='w-full h-[250px] object-cover rounded-lg' alt='project' />
      <Link to={card.link} >
      ...
      {isDescriptionShowing && <p className='text-center opacity-40'>{card.description}</p>}
